...
public class BankServiceImpl implements BankService {
    ...
    private final Validation<String> validateName = name -> {
        if(name == null || name.isBlank()) throw new ValidationException(" Validate Name is required");
    };

    private final Validation<String> validateEmail = email -> {
      if(email == null || !email.contains("@")) throw new ValidationException("Validate Email is required");
    };

    private final Validation<String> validateType = type -> {
        if(type == null || (type.contains("SAVINGS") || type.contains("CURRENT"))) throw new ValidationException("Validate type is required");
    };
...
//        Transaction fromTransaction = new Transaction(from.getAccountNumber(), amount, UUID.randomUUID().toString(), note, LocalDateTime.now(), Type.WITHDRAW);
//        transactionRepository.add(fromTransaction);
    ...
    public List<Transaction> getStatement(String account) {
        return transactionRepository.finByAccount(account).stream()
